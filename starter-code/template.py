...
def call_anthropic(
    ...
    latency = time.time() - start_time
    response_text = "".join(part.text for part in response.content)
    usage = {
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_response, gpt4o_latency, gpt4o_usage = call_openai(prompt)
    gpt4o_mini_response, gpt4o_mini_latency, gpt4o_mini_usage = call_openai(
        prompt, model=OPENAI_MINI_MODEL
    )
    gemini_response, gemini_latency, gemini_usage = call_gemini(prompt)

    return {
        "gpt4o": {
            "response": gpt4o_response,
            "latency": gpt4o_latency,
            "cost": _cost("gpt-4o", gpt4o_usage["input_tokens"], gpt4o_usage["output_tokens"]),
        ...
        "gpt4o_mini": {
            ...
            "cost": _cost("gpt-4o-mini", gpt4o_mini_usage["input_tokens"], gpt4o_mini_usage["output_tokens"]),
        ...
        "gemini_flash": {
            ...
            "cost": _cost("gemini-2.5-flash", gemini_usage["input_tokens"], gemini_usage["output_tokens"]),
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for model_key, model_label in model_labels.items():
            ...
            lines.append(
                f"| {prompt} | {model_label} | {_truncate(stats['response'])} | "
                f"{stats['latency']:.2f}s | {stats['input_tokens']}/{stats['output_tokens']} | {stats['cost']:.6f} |"
            )
    return "\n".join(lines)
